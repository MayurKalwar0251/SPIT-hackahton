...
import { 
  Lightbulb, Thermometer, Camera, BarChart2, Shield, Wind, 
  Bell, Lock, Unlock, AlertTriangle, Sun, Moon, Cloud, Power,
  DoorOpen, Droplet, Zap, Activity, Mic
} from 'lucide-react'
import Swal from 'sweetalert2'

// Mock data
const energyData = [
  { time: '00:00', usage: 2 }, { time: '04:00', usage: 1 },
  { time: '08:00', usage: 3 }, { time: '12:00', usage: 5 },
  { time: '16:00', usage: 4 }, { time: '20:00', usage: 3 },
  { time: '23:59', usage: 2 },
]
...
export default function Component() {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      setTemperature(prev => Math.round((prev + (Math.random() - 0.5)) * 10) / 10)
      setHumidity(prev => Math.min(100, Math.max(0, prev + Math.floor(Math.random() * 3) - 1)))
      if (Math.random() > 0.95) {
        const newAlert = Motion detected in ${rooms[Math.floor(Math.random() * rooms.length)]}
  ...
  const toggleCamera = async () => {
    if (isCameraOn) {
      ...
      try {
        ...
        showAlert("Failed to access the camera. Please check your permissions.")
      }
    }
  }
        ...
        >
          {activeTab === 'home' && (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              <motion.div className="bg-gray-800 p-6 rounded-lg" variants={fadeInVariants}>
                ...
                <div className="flex items-center justify-between">
                  ...
                  <div>
                    <button 
                      ...
                      onClick={() => setTemperature(prev => Math.max(16, prev - 0.5))}
                    >
                      -
                    </button>
                    <button 
                      className="bg-red-500 text-white px-3 py-1 rounded-r"
                      onClick={() => setTemperature(prev => Math.min(30, prev + 0.5))}
                    >
                      +
                    </button>
                  </div>
          ...
          {activeTab === 'energy' && (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <motion.div className="bg-gray-800 p-6 rounded-lg" variants={fadeInVariants}>
                <h2 className="text-xl font-semibold mb-4">Real-time Energy Usage</h2>
              ...
              <motion.div className="bg-gray-800 p-6 rounded-lg" variants={fadeInVariants}>
                <h2 className="text-xl font-semibold mb-4">Energy Consumption by Device</h2>
                <ResponsiveContainer width="100%" height={300}>
                  <AreaChart
                    data={[
              ...
              <motion.div className="bg-gray-800 p-6 rounded-lg col-span-1 md:col-span-2" variants={fadeInVariants}>
                ...
                <ul className="list-disc  pl-5">
                  ...
                  <li>Unplug devices when not in use to avoid phantom energy consumption</li>
                </ul>
              </motion.div>
            </div>
          )}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <motion.div className="bg-gray-800 p-6 rounded-lg" variants={fadeInVariants}>
                ...
                <div className="mt-4 flex flex-col sm:flex-row justify-between gap-2">
                  <button 
                    className={${isCameraOn ? 'bg-red-500' : 'bg-blue-500'} text-white px-4 py-2 rounded w-full sm:w-auto}
                    onClick={toggleCamera}
                  >
                    {isCameraOn ? 'Turn Off Camera' : 'Turn On Camera'}
                  </button>
                  <button className="bg-green-500 text-white px-4 py-2 rounded w-full sm:w-auto" onClick={handleTalk}>
              ...
              <motion.div className="bg-gray-800 p-6 rounded-lg" variants={fadeInVariants}>
                ...
                <div className="flex items-center justify-between">
                  ...
                  <motion.button
                    ...
                    whileTap={{ scale: 0.9 }}
                  >
                    {isLocked ? <Lock className="w-8 h-8" /> : <DoorOpen className="w-8 h-8" />}
                  </motion.button>
                </div>
                <div className="mt-4">
          ...
          {activeTab === 'environment' && (
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
              {environmentalData.map((roomData, index) => (
                <motion.div key={index} className="bg-gray-800 p-6 rounded-lg" variants={fadeInVariants}>
                  <h2 className="text-xl font-semibold mb-4">{roomData.name}</h2>
